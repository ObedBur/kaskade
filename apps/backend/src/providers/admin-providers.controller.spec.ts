...
describe('AdminProvidersController', () => {
  ...
  it('approve should call service approve', async () => {
    ...
    await expect(controller.approve('app-id')).resolves.toEqual({
      status: 'APPROVED',
    });
  ...
  it('reject should call service reject', async () => {
    ...
    await expect(controller.reject('app-id')).resolves.toEqual({
      status: 'REJECTED',
    });
  ...
  it('assignServices should call service assignServices', async () => {
    ...
    await expect(controller.assignServices('p-id', dto)).resolves.toEqual({
      id: 'p-id',
    });
    expect(mockProvidersService.assignServices).toHaveBeenCalledWith(
      'p-id',
      dto,
    );
  });
  ...
  it('removeService should call service removeService', async () => {
    ...
    await expect(controller.removeService('p-id', 's-id')).resolves.toEqual({
      id: 'p-id',
    });
    expect(mockProvidersService.removeService).toHaveBeenCalledWith(
      'p-id',
      's-id',
    );
  });
