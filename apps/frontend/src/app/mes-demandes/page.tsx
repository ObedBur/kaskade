...
function PremiumPaymentModal({
  ...
  const [operator, setOperator] = useState<
    "AIRTEL" | "ORANGE" | "MPESA" | null
  >(null);
  const [phoneNumber, setPhoneNumber] = useState("");
  const [phase, setPhase] = useState<"INITIAL" | "INITIATING" | "WAITING_USSD">(
    "INITIAL",
  );
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      setCountdown(60);
      toast.info("Consultez votre téléphone pour confirmer le paiement.", {
        id: "premium-payment",
        duration: 60000,
      });
    } catch (err: any) {
      setPhase("INITIAL");
      toast.error(
        err.response?.data?.message || "Erreur lors du paiement premium.",
      );
    }
  };
    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-chocolat/80 backdrop-blur-xl">
      ...
      >
        <div className="mb-6 flex items-start justify-between gap-4">
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-ocre">
              Paiement premium
            </p>
            <h3 className="mt-1 text-2xl font-black uppercase tracking-tighter text-chocolat">
          ...
          <button
            ...
            className="rounded-2xl bg-zinc-100 p-3 text-chocolat disabled:opacity-50"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
...
export default function MesDemandesPage() {
  ...
  const confirmReschedule = async (plan: any) => {
    ...
    try {
      ...
      toast.success(
        "Votre demande de report a été transmise ! Un expert va confirmer le changement.",
        { id: loadingToast },
      );
      fetchRequests();
    } catch (err) {
  ...
  const filteredRequests = requests.filter((req) => {
    ...
    if (activeFilter === "EN ATTENTE")
      return ["PENDING", "APPROVED", "ACCEPTED"].includes(req.status);
    if (activeFilter === "EN COURS")
      return ["IN_PROGRESS", "AWAITING_FINAL"].includes(req.status);
  ...
  return (
    <main className="bg-[#FCFBF7] min-h-screen font-sans selection:bg-[#BC9C6C] selection:text-white overflow-x-hidden">
      ...
      <div className="pt-32 md:pt-40 pb-24 max-w-[1400px] mx-auto px-4 sm:px-10">
        ...
        >
          <h2 className="text-[#321B13]/40 text-xs font-bold uppercase tracking-[0.3em] mb-4">
            SUIVI DES SERVICES
          </h2>
          ...
          <p className="text-[#321B13]/70 max-w-lg text-sm md:text-base leading-relaxed border-l-2 border-[#BC9C6C] pl-6">
            Gérez vos demandes et sécurisez vos paiements. Nous protégeons vos
            fonds jusqu'à la validation finale.
          </p>
        </motion.section>
          ...
          <div className="flex flex-wrap items-center gap-2 md:gap-4">
            {["TOUTES", "EN ATTENTE", "EN COURS", "TERMINÉES"].map((filter) => (
              <button
                ...
                className={`text-[10px] font-black uppercase tracking-widest px-6 py-3 transition-all duration-300 ${
                  activeFilter === filter
          ...
          <AnimatePresence mode="popLayout">
            {filteredRequests.length > 0 ? (
              filteredRequests.map((req, index) => (
                ...
                >
                  ...
                  <div className="relative h-48 w-full overflow-hidden shrink-0">
                    <Image
                      src={getServiceImageUrl(req.service)}
                    ...
                    <div className="absolute top-4 left-4">
                      {req.status === "PENDING" && (
                        <span className="bg-white/90 backdrop-blur-md text-chocolat text-[8px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full border border-white/20">
                          En attente
                        </span>
                      )}
                      {req.status === "IN_PROGRESS" && (
                        <span className="bg-ocre text-white text-[8px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full flex items-center gap-1.5 shadow-lg shadow-ocre/20">
                          <Play className="w-2.5 h-2.5" /> En cours
                        </span>
                      )}
                      {req.status === "COMPLETED" && (
                        <span className="bg-green-500 text-white text-[8px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full flex items-center gap-1.5">
                          <CheckCircle2 className="w-2.5 h-2.5" /> Terminée
                        </span>
                      )}
                      {req.status === "ACCEPTED" && (
                        <span className="bg-blue-500 text-white text-[8px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full border border-white/20">
                          Acceptée
                        </span>
                      )}
                    </div>
                    ...
                    <div className="absolute bottom-4 right-4">
                      <p className="text-white text-xl font-black tracking-tighter">
                        {req.price ? `$${req.price}` : "Prix à valider"}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    <div className="flex items-center gap-2 mb-3">
                      <span className="text-[8px] font-black text-ocre uppercase tracking-[0.2em]">
                    ...
                    <p className="text-chocolat/50 text-[10px] font-bold uppercase tracking-widest mb-4">
                      Artisan :{" "}
                      <span className="text-chocolat">
                        {req.provider?.fullName || "Recherche..."}
                      </span>
                    </p>
                    ...
                    {req.scheduleFrequency && (
                      <div className="mt-auto pt-4 border-t border-zinc-50 flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-[7px] font-black text-chocolat/30 uppercase tracking-widest">
                            Abonnement
                          </p>
                          <p className="text-[10px] font-black text-chocolat uppercase">
                            {req.scheduleFrequency} • {req.scheduleDay}
                          </p>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="mt-6 flex items-center justify-between">
                      <span className="text-[9px] font-black text-chocolat/20 uppercase tracking-[0.2em]">
                        ID: {req.id.split("-")[0]}
                      </span>
              ...
              <div className="col-span-full py-32 text-center bg-white border border-[#321B13]/5 rounded-[40px]">
                ...
                <h3 className="text-sm font-black text-[#321B13]/30 uppercase tracking-widest">
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
            ...
            >
              ...
              <div className="relative h-64 shrink-0">
                <Image
                  src={getServiceImageUrl(selectedRequest.service)}
              ...
              <div className="flex-1 overflow-y-auto p-8 sm:p-12 space-y-8 custom-scrollbar">
                <div>
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <span className="text-chocolat/30 text-[10px] font-bold uppercase tracking-widest">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-5 bg-off-white rounded-3xl border border-zinc-100">
                    <p className="text-[9px] font-black text-chocolat/30 uppercase tracking-widest mb-2">
                      Artisan Assigné
                    </p>
                    <div className="flex items-center gap-3">
                      ...
                      <p className="text-xs font-black text-chocolat">
                  ...
                  <div className="p-5 bg-off-white rounded-3xl border border-zinc-100 text-right">
                    <p className="text-[9px] font-black text-chocolat/30 uppercase tracking-widest mb-1">
                      Montant Total
                    </p>
                ...
                <div className="space-y-6">
                  <div className="flex items-start gap-4">
                    <div className="p-3 bg-zinc-50 rounded-2xl">
                      <MapPin className="w-5 h-5 text-chocolat/40" />
                    </div>
                    <div>
                      <p className="text-[9px] font-black text-chocolat/30 uppercase tracking-widest">
                        Adresse d'intervention
                      </p>
                      <p className="text-sm font-bold text-chocolat">
                        {selectedRequest.address || "À préciser avec l'artisan"}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-4">
                    <div className="p-3 bg-zinc-50 rounded-2xl">
                      <Calendar className="w-5 h-5 text-chocolat/40" />
                    </div>
                    <div>
                      <p className="text-[9px] font-black text-chocolat/30 uppercase tracking-widest">
                        Planning & Fréquence
                      ...
                      <p className="text-sm font-bold text-chocolat">
                        {selectedRequest.scheduleFrequency
                          ? `Abonnement ${selectedRequest.scheduleFrequency}`
                          : "Intervention ponctuelle"}
                        {selectedRequest.scheduleDay &&
                          ` • Chaque ${selectedRequest.scheduleDay} à ${selectedRequest.scheduleTime}`}
                      </p>
                  ...
                  {selectedRequest.notes && (
                    <div className="flex items-start gap-4">
                      <div className="p-3 bg-zinc-50 rounded-2xl">
                        <FileText className="w-5 h-5 text-chocolat/40" />
                      </div>
                      <div>
                        <p className="text-[9px] font-black text-chocolat/30 uppercase tracking-widest">
                          Notes & Instructions
                        </p>
                        <p className="text-sm text-chocolat/70 leading-relaxed italic">
                          "{selectedRequest.notes}"
                        </p>
                      </div>
                    </div>
                ...
                <div className="pt-8 border-t border-zinc-100 flex flex-wrap gap-4">
                  {selectedRequest.scheduleFrequency &&
                    selectedRequest.status === "PENDING" && (
                      <div className="w-full rounded-2xl border border-ocre/20 bg-ocre/5 p-5 text-sm font-bold text-chocolat">
                        Prix premium en attente de validation par
                        l'administration.
                      </div>
                    )}
                  {selectedRequest.scheduleFrequency &&
                    ...
                    selectedRequest.price && (
                      <button
                        onClick={() => {
                          setPaymentRequest(selectedRequest);
                  ...
                  {!selectedRequest.scheduleFrequency &&
                    selectedRequest.status === "ACCEPTED" && (
                      <button
                        onClick={() => {
                          handlePayment(selectedRequest.id, "deposit");
                          setShowDetailModal(false);
                        }}
                        className="flex-1 bg-ocre text-white py-5 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl shadow-ocre/20 hover:scale-[1.02] transition-all"
                      >
                  ...
                  {selectedRequest.status === "AWAITING_FINAL" && (
                    <button
                      onClick={() => {
                        handlePayment(selectedRequest.id, "final");
                        setShowDetailModal(false);
                      }}
                  ...
                  <button
                    onClick={() => {
                      handleReschedule(selectedRequest);
                      setShowDetailModal(false);
                    }}
