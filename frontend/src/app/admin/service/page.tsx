  ...
  return (
    <div className="p-8 md:p-12 space-y-12">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h1 className="text-4xl font-black text-[#321B13] tracking-tighter uppercase leading-none">
        ...
        <div className="flex items-center gap-4 w-full md:w-auto">
          <div className="relative flex-1 md:w-72">
            ...
            <input 
              ...
              className="w-full bg-white border border-gray-100 py-3.5 pl-12 pr-4 rounded-xl text-sm focus:outline-none focus:border-[#BC9C6C] transition-all"
            />
          </div>
          <button 
            onClick={() => handleOpenModal()}
            className="bg-[#321B13] text-white px-8 py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-3 hover:bg-[#BC9C6C] transition-all shadow-xl shadow-[#321B13]/5"
          >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        {[
          ...
          <div key={i} className="bg-white p-6 border border-gray-50 rounded-2xl flex items-center gap-5">
            <div className={`w-12 h-12 flex items-center justify-center rounded-xl ${stat.color}`}>
              <stat.icon className="w-5 h-5" />
            </div>
            <div>
              <p className="text-2xl font-black text-[#321B13]">{stat.value}</p>
      ...
      <div className="bg-white border border-gray-100 rounded-3xl overflow-hidden shadow-sm">
        <table className="w-full text-left">
          <thead className="bg-gray-50/50 border-b border-gray-100">
            <tr>
              <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-gray-400">Service</th>
              <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-gray-400">Catégorie</th>
              <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-gray-400">Prix Moyen</th>
              <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-gray-400">Statut</th>
              ...
              <tr key={service.id} className="hover:bg-gray-50/30 transition-colors group">
                <td className="px-8 py-6">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-gray-100 rounded-xl flex items-center justify-center text-gray-400 group-hover:bg-[#BC9C6C]/10 group-hover:text-[#BC9C6C] transition-all">
                      <Briefcase className="w-5 h-5" />
                    </div>
                    <div>
                      <p className="text-sm font-black text-[#321B13] uppercase tracking-tight">{service.name}</p>
                      <p className="text-[10px] text-gray-400 line-clamp-1 max-w-xs">{service.description || "Aucune description"}</p>
                    </div>
                  </div>
                </td>
                <td className="px-8 py-6">
                  <span className="text-[10px] font-black bg-gray-100 px-3 py-1.5 rounded-full uppercase tracking-widest text-gray-500">
                    {service.category}
                  </span>
                </td>
                <td className="px-8 py-6">
                  <p className="text-sm font-black text-[#321B13]">{service.price.toLocaleString()} FC</p>
                </td>
                <td className="px-8 py-6">
                  <div className={`flex items-center gap-2 text-[10px] font-black uppercase tracking-widest ${service.isActive ? "text-green-600" : "text-red-500"}`}>
                    <div className={`w-1.5 h-1.5 rounded-full ${service.isActive ? "bg-green-600 animate-pulse" : "bg-red-500"}`} />
              ...
              <div className="p-10 space-y-8">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="grid grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Prix Indicatif (FC)</label>
                      <input 
                        type="number" 
                        required
